...
- 2026-09-25 — Surface the Graph error body when `sendMail` fails.
  ...
  - Once the cause was confirmed the diagnostics were trimmed back: the catch block no longer echoes the status, the sender or the 404 explainer. What remains is one line printing `$_.ErrorDetails.Message` before the rethrow — the Graph error code is the part that isn't recoverable from the log otherwise, and the rest is written down in `README.md` instead of printed on every failure.
  - Confirmed on 2026-09-25: the 404 was the `mailFrom` domain (`admin@mycloudgurucom.com`, a domain the tenant does not own). Corrected to `admin@mycloudguru.com` in `pipelines/azure-pipelines.yml`; a dry run now reaches `Report sent.` and the HTML report arrives. The `Mail.Send` grant and the application access policy were never at fault, as the 403-vs-404 split predicted.
...
- 2026-09-24 — Fix `Argument types do not match` in `Sync-ResourceGroupTags`; stop returning bare collections from functions.
  ...
  - Fix: the three `return ,@($changes)` sites now return `[pscustomobject]@{ Changes = $changes }` — a scalar envelope holding the `List` itself. No array is built, and a scalar return cannot be unrolled by the pipeline, which is what the `,@()` wrapper was there to prevent. Callers use `$sync.Changes`; `List[object]` supports `.Count` and `foreach`, so nothing else changed.
  - `Resolve-MailRecipients` had the identical `return ,@($out)` construct and would have failed the same way on an empty recipient list. It now returns the `List` and the single call site re-collects with `@(...)` — the pattern `Get-SubscriptionsUnderManagementGroup` already uses in both scripts.
  - Convention: **do not return a bare collection from a function in these scripts.** PowerShell unrolls collections written to the output stream, so a 0- or 1-item result reaches the caller as `$null` or a lone object. Either wrap it in a scalar object (when the caller wants it intact) or `@()` it at the call site. Do not reach for `,@(...)`.
  - `trap` at script scope prints the exception message, the failing line number and source line, and the PowerShell stack trace before rethrowing. This is what located the bug; keep it — ADO otherwise surfaces the message alone, with no position.
  - Also removed, while the cause was still unknown, three needless .NET overload-resolution sites in the report builder: `System.Text.StringBuilder` → `List[object]` joined at the end; `[System.Net.WebUtility]::HtmlEncode` → chained `-replace` (`&` first, so the entities it emits aren't re-encoded); `[datetime]::UtcNow.ToString(<format>)` → `Get-Date -Format`. None of these was the fault, but the simpler forms are no worse.
  - Summary rows iterate `$Stats.GetEnumerator()` rather than `$Stats[$key]`: `OrderedDictionary` exposes both `Item[int]` and `Item[object]`, so indexing it is the ambiguous form and enumerating it is the safe one. Noted in a comment so it isn't "simplified" back.
