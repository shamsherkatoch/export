...
- 2026-09-24 — Remove .NET overload-resolution surfaces from the report builder; add a `trap` that reports the failing line.
  - Symptom: the reconciliation task exited 1 with only `##[error]Argument types do not match` — a .NET `ArgumentException` from overload resolution, with no line number, no statement, no stack. The sync task, which shares the token/Graph/CSV code, ran clean, so the fault was in code unique to the reconciliation script.
  - `scripts/Invoke-TagReconciliation.ps1` — `trap` at script scope prints the exception message, the failing line number and source line, and the PowerShell stack trace before rethrowing. Without it an overload failure is undiagnosable from an ADO log, which is what made this cost a run to chase.
  - Report builder no longer uses `System.Text.StringBuilder` (`Append` carries ~30 overloads) — a `List[object]` of strings joined at the end, matching the pattern already proven in `Sync-CsvWithAzure.ps1`. `[System.Net.WebUtility]::HtmlEncode` replaced with chained `-replace` (`&` first, so the entities it emits aren't re-encoded). `[datetime]::UtcNow.ToString(<format>)` replaced with `Get-Date -Date ([datetime]::UtcNow) -Format`.
  - Summary rows still iterate `$Stats.GetEnumerator()` rather than `$Stats[$key]`: `OrderedDictionary` exposes both `Item[int]` and `Item[object]`, so indexing it is the ambiguous form and enumerating it is the safe one. Noted in a comment so it isn't "simplified" back.
  - Not yet confirmed as the fix — the constructs removed were the plausible candidates, not a reproduced root cause. The `trap` is what settles it on the next run.
